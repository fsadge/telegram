...
```bash
/start                    # 开始对话，触发人机验证
按顺序点击Emoji           # 180秒内按提示方向依次点击5个目标Emoji按钮
...
为拦截广告/刷屏机器人，机器人默认开启 **Emoji序列点击验证**：用户首次联系时收到九宫格 Emoji 按钮，需在限时内按提示的目标序列依次点击 **5 个目标**，验证通过前的任何消息都不会转发给管理员。点击方向每次随机，可能是「从左往右」也可能是「从右往左」，以提示文案为准。
...
| 目标数量 | 每次 5 个目标 Emoji，从九宫格中随机抽取 |
| 点击方向 | 每次随机「从左往右」或「从右往左」，提示文案与出题顺序一致，无法背题 |
...
| 仅限私聊 | 验证只在私聊中签发与点击：群组/频道内的消息既不验证也不转发，只引导用户私聊 |
...
>
> 🚫 **如需关闭**：设置 `ENABLE_CAPTCHA=false` 即可恢复为不验证的原始行为（群组消息也会重新按原逻辑转发）。
>
> 👥 **群聊说明**：人机验证无法在群组内完成，因此开启验证时，群组/频道里发来的消息不会被转发给管理员，只会提示用户私聊机器人。
...
### v1.4.0 (Latest) - 🎯 验证目标增至5个，点击方向随机
- ⬆️ **目标数量**: 每次验证由 3 个 Emoji 增加为 5 个，降低被脚本蒙对的概率
- ↔️ **方向随机**: 每次随机「从左往右」或「从右往左」，提示文案与展示序列同步，无法固定套路作答
- 🐛 **修复群聊验证失败**: 人机验证只应在私聊中进行。此前群组里发消息也会下发验证题，而群内点击会被判定为“请在私聊中完成验证”，导致无法通过
- 👥 **群聊新行为**: 开启验证时，群组/频道消息不再转发，只引导用户私聊机器人（关闭验证后恢复原逻辑）
- ✅ **进度标记**: ✅ 进度按当前方向标注在正确的展示位置
